...
(async()=>{
 ...
 const r=await p.evaluate(()=>{
   ...
   const want=['sword','great','hammer','bow','staff','wand','claw','boomer'];
   for(const id of T.EQ_IDS){ const E=T.EQUIP[id]; if(!E||E.slot!==0) continue;
     const t=T.WEP_TYPE[E.tn]; if(!t||want.indexOf(t.id)<0) continue;
     if(out.find(o=>o.t===t.id)) continue;
     const g=T.dbg.wgrip(id);
     out.push({t:t.id, key:T.dbg.artKey?T.dbg.artKey(id):'?', x:+g.x.toFixed(3), y:+g.y.toFixed(3), rot:+g.rot.toFixed(2)});
   }
   return out;
 });
