...
(async()=>{
  ...
  const res=await p.evaluate(async()=>{
    ...
    const targets=['dagger','claw','tome','orb','bell','shuri','hammer','scythe','staff','cannon',
                   'halberd','trident','kusari','umbrella','broom','fishrod','brush','starst'];
    const out=[];
    T.S.eq=T.S.eq||{};
    for(const wid of targets){
      ...
      const s0=H[0], a0=H[1], g0=H[3];
      let best=null;
      for(let ds=0; ds<9; ds++){
        const sc = s0 * (0.80 + ds*0.12);           // 크기 0.80~1.76배
        for(let da=-8; da<=8; da++){
          const ang = a0 + da*0.09;                 // 각도 ±0.72 rad
          H[0]=sc; H[1]=ang;
          const m=measure(id);
          if(!m) continue;
          if(m.clip) continue;
          if(m.len<0.34 || m.len>0.75) continue;
          if(m.grip<160) continue;
          const score = m.face*10 + Math.abs(m.len-0.46)*20 + Math.abs(da)*0.35 + Math.abs(ds-1.67)*0.30;
          if(!best || score<best.score) best={score, s:+sc.toFixed(3), a:+ang.toFixed(3), ...m};
        }
      }
      H[0]=s0; H[1]=a0;
      out.push({wid, before:{s:s0,a:a0}, best});
    }
    return out;
  });
  console.log('무기        크기      각도     길이/키  얼굴%  주먹');
  const fix=[];
  for(const r of res){
    ...
    fix.push([r.wid, b2.s, b2.a]);
  }
  require('fs').writeFileSync('/tmp/fix.json', JSON.stringify(fix));
  await b.close();
})();
