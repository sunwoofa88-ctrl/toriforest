...
(async()=>{
  ...
  for(const elem of wants){
    ...
    if(!wKey){ console.log('SKIP', elem); continue; }
    await p.evaluate((wKey)=>{
      const T=window.__TORI;
      T.S.eq=T.S.eq||{}; T.S.eq[wKey]=1; T.eqSet('w', wKey);
      T.S.eqPlus=T.S.eqPlus||{}; T.S.eqPlus[wKey]=0; // +0 — no enhancement at all
      T.P.fx=1; T.P.fy=0; T.P.facing=1;
    }, wKey);
    await p.evaluate(()=>{ const T=window.__TORI; T.doAttack(T.P.x+200, T.P.y); });
    await p.waitForTimeout(90);
    fs.writeFileSync('/tmp/atk_zero_'+elem+'.png', await p.screenshot());
    await p.waitForTimeout(400);
  }

  console.log('weapons:', JSON.stringify(found));
